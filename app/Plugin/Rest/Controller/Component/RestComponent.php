                    ...
                    foreach ($Controller->components['Rest.Rest']['actions'] as $action => $vars) {
                        if (!in_array($action, $Controller->methods)) {
                            $this->debug(sprintf(
                                            ...
                                            'warnings in the readme file, or just did not implement the "%s" action in the "%s" controller yet', $Controller->name, $action, $action, $Controller->name
                                    ));
                            continue;
                        }
                        $saveVars = array();
                        ...
                        $exposeVars = array_merge(
                                $this->settings['exposeVars']['*'], isset($this->settings['exposeVars'][$action]) ? $this->settings['exposeVars'][$action] : array()
                        );
                            ...
                            if (isset($vars[$exposeVar])) {
                                ...
                                if (isset($this->settings['defaultVars'][$action][$exposeVar])) {
                                    ...
                                    return $this->abort(sprintf(
                                                            ...
                                                            '%s->components->Rest.Rest->actions[\'%s\'][\'%s\'] = %s', $exposeVar, $action, $className, $action, $exposeVar, $example
                                                    ));
                                }
                            }
                        }
    ...
    public function isActive() {
        if ($this->isActive === null) {
            ...
            return $this->isActive = in_array(
                    !empty($this->Controller->request->params['ext']) ? $this->Controller->request->params['ext'] : null, $this->settings['extensions']
            );
        }
