...
<div id="account">
	...
	function fungetLotterytype() {
	   jQuery('#lotterytype').load('<?php echo $this->Html->url(array('controller' => 'LotteryTickets', 'action' => '/lotteryheader/'.$id.'')); ?>', function() {});
	}
	fungetLotterytype();
	</script>
     </div>

     <?php echo $this->Form->create('LotteryTicket', array('url' => array('controller' => 'LotteryTickets', 'action' => 'admin_ticket_print')));?>

     <h3 class="fontbld"><?php echo __('Select Lottery Number'); ?></h3>
	
     <fieldset style="background:#E4E5E6;width:40%;">
	...
	<fieldset class="pT10 pL10 pR10">      
	   <?php echo $this->Form->input('reset',array('div'=> false, 'style'=>'width:30%;float:left;','id'=>'resetlotteryball','type'=>'reset','label'=>false,'class'=>'button','value'=>'Reset Ticket'));?>
	   <?php echo $this->Form->input('Generate Ticket',array('div'=>false, 'style'=>'width:30%;float:right;','id'=>'generate_ticket_button','type'=>'submit','label'=>false,'class'=>'button','value'=>'Generate Ticket'));?>	      
	</fieldset>
	   ...
	   jQuery(document).ready(function(){
	     ...
	     jQuery('#generate_ticket_button').click(function(){	       
		  ...
		  if(confirm("It will take a lottery fee from your account in generating a Ticket.Click OK to Contine and Cancel to Return.")){
	          ...
	          if(lottery_field ==''){
		     jQuery('#ticket_id').css('border','3px solid red');
		  } else {
		     jQuery('#ticket_id').css('border','1px solid #000000');
		  }
		  if(lottery_field ==''){
		     jQuery('#stuff_ball').css('border','3px solid red');
		  }else {
