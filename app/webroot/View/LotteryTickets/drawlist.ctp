<table style="margin-bottom:10px;" class="items">
   <tr>				
      <th class="itemsTab minwidth">LOTTERY NUMBER DRAW WINNER LIST</th>
   </tr>
</table>
...
<?php if(!empty($winnerData)) { ?>
    ...
    <?php foreach ($winnerData as $windatekey => $winnersAll) { ?>
	   ...
	   <?php if(isset($windatekey) && !is_numeric($windatekey)){?>
	    ...
	    <table class="items">
		<tr>				
		   <th class="itemsTab minwidth"><?php if(isset($windatekey) && $windatekey!='') { echo date('d M Y H:i',strtotime($windatekey));} ?> </th>
		</tr>
	    </table>
		  ...
		  <?php echo $this->Form->create();?>
		     ...
		     <table class="marginTable default-table">
				...
				<tr>
					...
					$imageSrc = $this->MyHtml->image(''.$imagepath.'', array('alt' => ''.$logo.'','class'=>'logosmall'));
					echo $this->MyHtml->spanLink(__(''.$imageSrc.''), array('action' => 'admin_view',$row['Lottery']['id'],'view'), array('class' => '')); ?>
				    </td>
