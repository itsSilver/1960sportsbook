...
<div id="" class="index">
    ...
    <table class="default-table">
	   ...
	   <tr>				
		...
		<!-- <th><?php echo $this->Paginator->sort('LotteryTicket.agent_id', __('AGENT ID')); ?></th> -->
		<th><?php echo $this->Paginator->sort('LotteryTicket.user_id', __('PLAYER ID')); ?></th>
		...
		?></th>  -->
		<th><?php echo $this->Paginator->sort('LotteryTicket.draw_date', __('REQUEST STATUS')); ?></th>
	    ...
	    <?php if (!empty($data)){ ?>
		    ...
		    <?php foreach ($data as $row){
			   ...
			   $user_id = $agent_id = $status = $ticket_number = $stuff_ball = $is_active = $is_stuff = $logo = $ticket_id = $lottery_id = $name = $lottery_type_name = $num_lott_ball = $lottery_fee = $draw_time = $is_stuff = $is_active = $prize_level = $jackpotnumber = $matchticketnumber = $winticketnumber = '';
