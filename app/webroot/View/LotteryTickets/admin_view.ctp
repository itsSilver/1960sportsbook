...
$win_ticket = $win_date = $draw_date = $player = $status = $ticket_number = $stuff_ball = $is_active = $is_stuff = $logo = $ticket_id = $lottery_id = $name = $lottery_type_name = $num_lott_ball = $lottery_fee = $draw_time = $is_stuff = $is_active = $prize_level = $jackpotnumber = $matchticketnumber = $winticketnumber = '';
?>
...
<div id="items">
	...
	<table class="marginTable default-table">
            <tr>				
		<th>Search Result:</th> 
		<th><?php echo $this->MyHtml->spanLink(__('Back'), array('controller' => '/'), array('class' => 'right')); ?></th>  
            </tr>
	</table>
	<?php } else { ?>
	<table class="marginTable default-table">
            <tr>				
		<th><?php echo $this->MyHtml->spanLink(__('All Tickets >> Ticket Detail'), array('action' => 'admin_tickets'), array('class' => 'left')); ?></th> 
		<th><?php echo $this->MyHtml->spanLink(__('Back'), array('action' => 'admin_tickets'), array('class' => 'right')); ?></th> 
            </tr>
	</table>
	<?php } ?>
	<br />
		...
		<tr>
		   <td colspan="2">
			 <label class="itemsTab fontbld"><?php echo __('User Lottery Detail'); ?></label>
		   </td>
		</tr>
		...
		<tr>
		    <td>
			<label><?php echo __('Ticket Number Status'); ?></label>
		   </td>
		   <td>
			<label>
			<?php
		...
		<tr>
		   <td colspan="2">
			 <label class="itemsTab fontbld"><?php echo __('Winning Lottery Detail'); ?></label>
		   </td>
		</tr>
		...
		<tr>
		    <td>&nbsp;</td>
		    <td>
		         <?php echo $this->MyHtml->spanLink(__('Print'), array('action' => 'admin_action',$ticket_id,'print'), array('class' => 'button')); ?>
		         <?php
			 if(isset($status) && $status==0 && $this->groupid!=8){
